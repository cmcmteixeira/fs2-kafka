...
import cats.{Applicative, Foldable, Functor, MonadThrow}
...
object KafkaAdminClient {
  ...
  def describeLogDirsWith[F[_]: Functor, G[_]: Foldable: Functor](
    ...
    withAdminClient(_.describeLogDirs(brokers.map(Integer.valueOf).asJava).allDescriptions()).map(
      _.asScala.view.map { case (i, m) => i.intValue() -> m.asScala.toMap }.toMap
    )
  ...
  def expireDelegationTokenWith[F[_]: Functor](
    ...
    withAdminClient(_.expireDelegationToken(hmac, opt).expiryTimestamp()).map(_.longValue())
  }
  ...
  def renewDelegationTokenWith[F[_]: Functor](
    ...
    withAdminClient(_.renewDelegationToken(hmac, options).expiryTimestamp()).map(_.longValue())
  }
  ...
  def describeClientQuotasWith[F[_]: Functor](
    ...
    withAdminClient(_.describeClientQuotas(filter).entities()).map(
      _.asScala.view.mapValues(_.asScala.view.mapValues(_.doubleValue()).toMap).toMap
    )
  ...
  def fenceProducersWith[F[_]: Functor, G[_]: Foldable](
    ...
    withAdminClient(_.fenceProducers(transactionalIds.asJava).all).void
  ...
  def metricsWith[F[_]: Functor](
    ...
    withAdminClient{ client =>
      val result = client.metrics().asScala.toMap
      KafkaFuture.completedFuture(result)
    }
  }
